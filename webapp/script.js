...
function backToMenu() {
    ...
    // Enhanced transition back to menu
    const mainMenu = document.getElementById('mainMenu');
    const formContainer = document.getElementById('formContainer');
...
function updateProgress() {
    ...
    // Update progress bar color and add feedback
    if (progress >= 100) {
...
function validateField(field) {
    ...
    if (field.required && !value) {
        ...
        // Enhanced specific field validations
        switch (fieldName) {
            case 'kode_sa':
    ...
    // Update field appearance with enhanced animations
...
function updateFieldAppearance(field, isValid, errorMessage = '') {
    ...
    if (field.value.trim() === '') {
        ...
        // Valid field - success state with enhanced animation
        ...
        // Invalid field - error state with enhanced feedback
...
function handlePhotoUpload(event) {
    ...
    if (!allowedTypes.includes(file.type)) {
        showError(errorElement, 'Format file tidak didukung. Gunakan JPG, PNG, atau WebP');
        event.target.value = '';
        return;
    }
...
function collectFormData(form) {
    ...
    data.activity_type = currentActivity;
